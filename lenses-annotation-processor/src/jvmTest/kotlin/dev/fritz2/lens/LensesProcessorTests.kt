...
class LensesProcessorTests {
            ...
            },
            {
                assertThat(compilationResult.kspGeneratedSources.find { it.name == "BarLenses.kt" })
                    .usingCharset(StandardCharsets.UTF_8).hasContent(
                        ...
                        |import kotlin.Int
                        |
                        |public fun Bar.Companion.bar(): GetterLens<Bar, Int> = lensOf(
                        |    "bar",
                        |    Bar::bar
                        |  )
                        |
                        |public fun <PARENT> GetterLens<PARENT, Bar>.bar(): GetterLens<PARENT, Int> = this + Bar.bar()
                        """.trimMargin()
                    )
            }
        )
    ...
    fun `lenses will throw error if lens fun's name is already in use`(description: String, kotlinSource: SourceFile) {
        val compilationResult = compileSource(kotlinSource)
    ...
    companion object {
        private val validCodeForSealedHierarchies = """
            ...
            |import kotlin.String
            |
            |public fun Foo.Companion.bar(): GetterLens<Foo, Int> = lensOf(
            |    "bar",
            |    Foo::bar
            |  )
            |
            |public fun <PARENT> GetterLens<PARENT, Foo>.bar(): GetterLens<PARENT, Int> = this + Foo.bar()
            |
            |public fun Foo.Companion.foo(): GetterLens<Foo, String> = lensOf(
            |    "foo",
            |    Foo::foo
            |  )
            |
            |public fun <PARENT> GetterLens<PARENT, Foo>.foo(): GetterLens<PARENT, String> = this + Foo.foo()
            |
            |public fun Foo.Companion.fooBar(): GetterLens<Foo, MyType> = lensOf(
            |    "fooBar",
            |    Foo::fooBar
            |  )
            |
            |public fun <PARENT> GetterLens<PARENT, Foo>.fooBar(): GetterLens<PARENT, MyType> = this +
            |    Foo.fooBar()
            |
            |public fun Foo.Companion.baz(): GetterLens<Foo, MyGenericType<Int>> = lensOf(
            |    "baz",
            |    Foo::baz
            |  )
            |
            |public fun <PARENT> GetterLens<PARENT, Foo>.baz(): GetterLens<PARENT, MyGenericType<Int>> = this +
            |    Foo.baz()
            """.trimMargin()
