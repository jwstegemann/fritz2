            ...
            div("flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0") {
                ...
                div(
                    """inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl
                        | transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6""".trimMargin()
                ) {
                    transition(
                        "ease-out duration-300",
                    ...
                    div("sm:flex sm:items-start") {
                        div(
                            // Fixme: rounded is not applied - why?
                    ...
                    div("mt-5 sm:mt-4 sm:flex sm:flex-row-reverse") {
                        button(
                            // FIXME: hover does not work! Why? (is intentionally red to be recognizable)
                            """w-full inline-flex justify-center rounded-md border border-transparent shadow-sm
                                | px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-red-800
                        ...
                        button(
                            """mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4
                                ...
                                | focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:mr-16
                                | sm:text-sm""".trimMargin()
                        ) {
                            type("button")
                            +"Cancel"
