...
tasks.register("versionToJs") {
    doLast {
        FileWriter(File(rootDir.resolve("www/src/scripts"), "version.js")).run {
            write("// generated by 'versionToJs' gradle task\n")
            write("export default \"${subprojects.find { it.name == "core" }?.version ?: ""}\"")
            flush()
            close()
        }
    }
